...
class TrackMyBabyController extends GetxController {
  ...
  // Get sorted children by age (ascending - youngest first)
  // Returns a list of tuples: (originalIndex, childData)
  List<MapEntry<int, ChildData>> get sortedChildrenByAge {
    final List<MapEntry<int, ChildData>> indexedChildren = [];
    for (int i = 0; i < allChildren.length; i++) {
      indexedChildren.add(MapEntry(i, allChildren[i]));
    }
    // Sort by birth date descending (newest/youngest first)
    indexedChildren.sort((a, b) => b.value.dob.compareTo(a.value.dob));
